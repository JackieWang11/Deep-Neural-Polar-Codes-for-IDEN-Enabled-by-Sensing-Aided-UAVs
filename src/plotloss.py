...
with tf.Session() as sess:
    ...
    saver.restore(sess, './ParamOfLoss/3/save_net.ckpt')
    ##################### train
    ...
    file = open("ParamOfLoss/loss_with3.txt", 'w')
    for i in range(len(epoch_losses)):
        s = str(epoch_losses[i]).replace('[', '').replace(']', '')  # 去除[],这两行按数据不同，可以选择
        s = s.replace("'", '').replace(',', '') + '\n'  # 去除单引号，逗号，每行末尾追加换行符
        file.write(s)
    file.close()
    # save_path = saver.save(sess, 'D:/workspace/cuijingwen/stage3_2/ParamOfLoss/3/save_net.ckpt')
