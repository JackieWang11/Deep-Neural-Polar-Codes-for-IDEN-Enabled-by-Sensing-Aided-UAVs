# for Fig2 close
# 注意！！！！！！！！！！！！！！！！！！！！！！！！！！ 该代码值适用于（128，32）和（128，96）的，（64，32）和（256，128）用forFig1close的代码，二者初始化不一样的
# iter=5
...
# llr_froz_vec=tf.Variable((np.float32(ffro)-0.5))
###########
# BP
...
loss=1*tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_out_label,logits=y_output))+\
      0.5*tf.reduce_mean(tf.square(Rate-tf.reduce_mean(Prob,axis=1)))#+0.1*tf.reduce_mean(Rate-tf.reduce_mean(Prob,axis=1))
...
with tf.Session() as sess:
    ...
    # saver.restore(sess, 'D:/cuijingwen/PycharmProjects/stage3_2/ParamofFig2/1283/0.34/save_net.ckpt')
    ##################### train
    ...
    # save_path = saver.save(sess, 'D:/cuijingwen/PycharmProjects/stage3_2/ParamofFig2/test2/0.34/save_net.ckpt')
    ...
    # print(np.shape(rr))
    # np.savetxt('reliability/32/'+str(N)+'.txt', rr)
