...
rho=tf.constant(0.65)
# rho=tf.Variable(1/2,dtype=tf.float32,constraint=lambda t: tf.clip_by_value(t, 0, 1))

#################### initial info/frozen position probability ################
# ##pa=np.tile(np.reshape(np.linspace(0,1,N),(1,N)),[batch_size,1])
...
# llr_froz_vec=tf.Variable(initializer([batch_size, N])*10)#tf.Variable((np.float32(ffro)-0.5))#  远离：tf.Variable(initializer([batch_size, N])*10) tf.Variable(np.float32(pa)*0.1)  靠近：tf.Variable((np.float32(ffro)-0.5))
llr_froz_vec=tf.Variable(np.float32(ffro)*0.1)
###########
# # ##### scaling parameters of DNN decoder
LV1 = tf.Variable(np.float32(np.ones((n_,N_,1))))
...
loss=1*tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_out_label,logits=y_output))+\
      0.5*tf.reduce_mean(tf.square(Rate-tf.reduce_mean(Prob,axis=1)))#+0.05*tf.reduce_mean(Rate-tf.reduce_mean(Prob,axis=1))+ 0.5*(P_tar-P_d)+1*tf.square(P_d-P_tar)
...
with tf.Session() as sess:
    ...
    # saver.restore(sess, 'D:/workspace/cuijingwen/stage3_2/Paramoffig1/512/yuan/0.65/save_net.ckpt')
    ...
    # save_path = saver.save(sess, 'D:/workspace/cuijingwen/stage3_2/Paramoffig1/512/yuan/0.65_2/save_net.ckpt')
    ...
    for i in range(n_steps):
        ...
        b0_mat = [0] * extranum
        ######################## #######
        y_pred, lab, ffro,ZZ,ZZZ,K_,P_del,RELIA = sess.run(fetches=[y_output, y_out_label, Fro,Z,Z2,kk,P_d,llr_froz_vec],
