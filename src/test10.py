...
with tf.Session() as sess:
    ...
    # saver.restore(sess, 'D:/workspace/cuijingwen/stage3_2/ParamofFig1/512/512_512/1/save_net.ckpt')
    saver.restore(sess, 'D:/workspace/cuijingwen/stage3_2/ParamofFig3/away/256/test/4/save_net.ckpt')
    ##################### train
    ...
    # # # #
        ## 记录loss
    #     epoch_losses.append(train_loss / n_steps)
    #
    # file = open("ParamOfLoss/loss_with5.txt", 'w')
    # for i in range(len(epoch_losses)):
    #     s = str(epoch_losses[i]).replace('[', '').replace(']', '')  # 去除[],这两行按数据不同，可以选择
    #     s = s.replace("'", '').replace(',', '') + '\n'  # 去除单引号，逗号，每行末尾追加换行符
    #     file.write(s)
    # file.close()
    # # #
    ...
    # save_path = saver.save(sess, 'D:/workspace/cuijingwen/stage3_2/ParamofFig1/512/512_512/1/save_net.ckpt')
    ...
    np.savetxt('reliabilityyuan2/keshan/' + str(N) + 's' + str(SNR_db) + 'e' + str(epoch) + '1.txt', RELIA)
