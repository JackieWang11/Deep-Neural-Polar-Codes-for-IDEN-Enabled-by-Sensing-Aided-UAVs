    ...
    with tf.Session() as sess:
        ...
        for j in range(epoch):
            ...
            for i in range(n_steps):
                ...
                H_ch4=np.reshape(H_ch3[:, 3, :], [batch_size, 1, -1])
                s_H_ch4=np.reshape(s_H_ch3[:, 3, :], [batch_size, 1, -1])
        ...
        # save_path = saver.save(sess, 'D:/cuijingwen/PycharmProjects/stage3/ParamOfSingleBNN/away/slot4/'+str(p)+'/save_net.ckpt')
        ...
        # print(np.shape(rr))
        np.savetxt('../Paramofpaperfig4/close/b3/t1.txt', rr)
...
def test3(p):
    with tf.Session() as sess:
        # tf.global_variables_initializer().run()
        saver.restore(sess, '../ParamOfSingleBNN/away/slot4/'+str(p)+'/save_net.ckpt')
        print('!!!!!!!!!!!!!!!!!!!!!!!! test3 !!!!!!!!!!!!!!!!!!!!!!!!!!\n')
        is_test = 1
        ...
        for i in range(t_step):
            ...
            #
            # H_ch4 = np.reshape(H_ch[:, 3, :], [batch_size, 1, -1])
            # s_H_ch4 = np.reshape(s_H_ch[:, 3, :], [batch_size, 1, -1])
