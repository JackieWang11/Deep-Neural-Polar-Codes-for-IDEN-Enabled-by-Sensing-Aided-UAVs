    ...
    with tf.Session() as sess:
        ...
        for j in range(epoch):
            ...
            for i in range(n_steps):
                ...
                H_ch4=np.reshape(H_ch3[:, 2, :], [batch_size, 1, -1])
                s_H_ch4=np.reshape(s_H_ch3[:, 2, :], [batch_size, 1, -1])
        ...
        # save_path = saver.save(sess, 'D:/cuijingwen/PycharmProjects/stage3/ParamOfSingleBNN/away/slot3/'+str(p)+'/save_net.ckpt')
        ...
        # print(np.shape(rr))
        np.savetxt('../Paramofpaperfig4/close/b2/t1.txt', rr)
...
def test2(p):
    with tf.Session() as sess:
        # tf.global_variables_initializer().run()
        saver.restore(sess, '../ParamOfSingleBNN/away/slot3/'+str(p)+'/save_net.ckpt')
        print('!!!!!!!!!!!!!!!!!!!!!!!! test2 !!!!!!!!!!!!!!!!!!!!!!!!!!\n')
        is_test = 1
        ...
        for i in range(t_step):
            ...
            #
            # H_ch4 = np.reshape(H_ch[:, 2, :], [batch_size, 1, -1])
            # s_H_ch4 = np.reshape(s_H_ch[:, 2, :], [batch_size, 1, -1])
    ...
    # rr=np.sum(chan_rel, axis=0)/batch_size
    # # print(np.shape(rr))
    # # np.savetxt('reliability/t1.txt', rr)
    #
    #
    # ##############################################
    # # print('x', zz)
