...
H=tf.placeholder(tf.complex64,shape=[None,time_slot,None])
s_H=tf.placeholder(tf.complex64,shape=[None,time_slot,None])
...
# pa=np.tile(np.reshape(np.linspace(1,0,N),(1,N)),[batch_size,1])
ffro=np.tile(np.reshape(pw,(1,N)),[batch_size,1])
# ffro=np.tile(np.reshape(FZlookup,(1,N)),[batch_size,1])
rho=tf.constant(1.)
initializer = tf.contrib.layers.xavier_initializer()
llr_froz_vec=tf.Variable((np.float32(ffro)-0.5))#  远离：tf.Variable(initializer([batch_size, N])*10) tf.Variable(np.float32(pa)*0.1)  靠近：tf.Variable((np.float32(ffro)-0.5))
...
def polar_encoder(x,Kr,isTest):
    ...
    return X,pos1,remain,pro,u,valid_len
...
def bp_algorithm(bp_iter_num,net_dict,RV,LV,n,N):
    ...
    for j in range(bp_iter_num):
        itr = 0
        # itr = j
        for i in range(n,0,-1):        # i决定的是第几层
            for phi in range(2**i):
                psi = int(np.floor(phi/2))
                if(np.mod(phi,2)!=0):                   #奇数才能计算下面
                    for omega in range(2 ** (n - i)):
                        net_dict["R_{0}{1}{2}".format(n+1-i,psi+2*omega*2**(i-1),0)] = RV[n-i,psi+2*omega*2**(i-1),itr]*fFunction(net_dict["L_{0}{1}{2}".format(n+1-i,psi+(2*omega+1)*2**(i-1),0)]+net_dict["R_{0}{1}{2}".format(n-i,psi+(2*omega+1)*2**(i-1),0)], net_dict["R_{0}{1}{2}".format(n-i,psi+2*omega*2**(i-1),0)])
                        net_dict["R_{0}{1}{2}".format(n+1-i,psi+(2*omega+1)*2**(i-1),0)] = net_dict["R_{0}{1}{2}".format(n-i,psi+(2*omega+1)*2**(i-1),0)]+RV[n-i,psi+(2*omega+1)*2**(i-1),itr]*fFunction(net_dict["L_{0}{1}{2}".format(n+1-i,psi+(2*omega)*2**(i-1),0)],net_dict["R_{0}{1}{2}".format(n-i,psi+(2*omega)*2**(i-1),0)])
        ...
        for i in range(1,n+1):
            for phi in range(2**i):
                ...
                if (np.mod(phi, 2) != 0):
                    for omega in range(2 ** (n - i)):
                        net_dict["L_{0}{1}{2}".format(n-i,psi+2*omega*2**(i-1),0)] = LV[n-i,psi+2*omega*2**(i-1),itr]*fFunction(net_dict["L_{0}{1}{2}".format(n+1-i,psi+2*omega*2**(i-1),0)],net_dict["L_{0}{1}{2}".format(n+1-i,psi+(2*omega+1)*2**(i-1),0)]+net_dict["R_{0}{1}{2}".format(n-i,psi+(2*omega+1)*2**(i-1),0)])
                        net_dict["L_{0}{1}{2}".format(n-i,psi+(2*omega+1)*2**(i-1),0)] = net_dict["L_{0}{1}{2}".format(n+1-i,psi+(2*omega+1)*2**(i-1),0)]+LV[n-i,psi+(2*omega+1)*2**(i-1),itr]*fFunction(net_dict["L_{0}{1}{2}".format(n+1-i,psi+2*omega*2**(i-1),0)],net_dict["R_{0}{1}{2}".format(n-i,psi+2*omega*2**(i-1),0)])
...
def generate_output(ori,Kr,zero,noise,isTest,H):
    ...
    ################ Polar Encoder ################
    encoded, fro_pos, rem,prob,u,va_len = polar_encoder(ori,Kr,isTest)
    temp_fro2 = get_fropos2(batch_size, fro_pos)
    encoded1 = tf.reshape(encoded, [batch_size, N])
    encoded2=tf.reshape(encoded1,[batch_size,time_slot, int(N/time_slot)])

    # encoded = tf.concat([encoded, zero], axis=0)
    # 可以补零，但此处不用补，暂时不写
    ################ Autoencoder ################
    # 4QAM
    x_enc1 = tf.sqrt(1 / 2) * (2 * encoded2 - 1) * tf.sqrt(SNR)
    x_enc2 = tf.reshape(x_enc1, [batch_size,time_slot, int(N/(time_slot*k)),2])
    ...
    # # channel
    # Noise1 = tf.random.normal([batch_size, 1, int(N / (time_slot * k)), 2], mean=0.0, stddev=0.5)  # 测试
    # Noise2 = tf.random.normal([batch_size, 1, int(N / (time_slot * k)), 2], mean=0.0, stddev=1.5)
    # Noise3 = tf.random.normal([batch_size, 1, int(N / (time_slot * k)), 2], mean=0.0, stddev=8.)
    # Noise4 = tf.random.normal([batch_size, 1, int(N / (time_slot * k)), 2], mean=0.0, stddev=12.)
    # Noise = tf.concat([Noise1, Noise2, Noise3,Noise4], axis=1)

    Noise = tf.random.normal([batch_size,time_slot,int(N/(time_slot*k)),2], mean=0.0, stddev=noisestd)
    ...
    return y_out,y_out_label,prob,fro_pos,va_len,P_del,(Z1_complex/s_H*H),(H/s_H)# 从y1开始可删
...
def train0(p):
    with tf.Session() as sess:
        ...
        # saver.restore(sess, "param/d_3_D_2/try1/save_net.ckpt")
        ##################### train
        ...
        for j in range(epoch):
            ...
            for i in range(n_steps):
                ...
                H_ch2=np.tile(np.reshape(H_ch,[batch_size,4,tx_N,1]),[1,1,1,int(4*N/(4*k*tx_N))])
                H_ch3=np.reshape(H_ch2,[batch_size,4,tx_N*int(4*N/(4*k*tx_N))])
                s_H_ch = sens_chan(s_tar_theta, s_d, batch_size)
                s_H_ch2 = np.tile(np.reshape(s_H_ch, [batch_size, 4, tx_N, 1]),[1, 1, 1, int(4*N / (4 * k * tx_N))])
                s_H_ch3 = np.reshape(s_H_ch2, [batch_size, 4, tx_N * int(4*N / (4 * k * tx_N))])

                H_ch4=np.reshape(H_ch3[:, 0, :], [batch_size, 1, -1])
                s_H_ch4=np.reshape(s_H_ch3[:, 0, :], [batch_size, 1, -1])


                b0_mat = [0] * extranum
                ######################## #######
                y_pred, lab, ffro, K, P_del,_loss,_= sess.run(fetches=[y_output, y_out_label, Fro, kk,P_d, loss,optimizer],
                             feed_dict={x: X_train, Kron: get_F_kron_n(n), H:H_ch4, s_H:s_H_ch4,noisestd: np.sqrt(0.5), zero_mat: b0_mat,
        ...
        # 记录神经网络参数
        # save_path = saver.save(sess, "D:/cuijingwen/PycharmProjects/stage3/ParamOfSingleBNN/away/slot1/p0.78/save_net.ckpt")
        # save_path = saver.save(sess, 'D:/cuijingwen/PycharmProjects/stage3/ParamOfSingleBNN/away/slot1/'+str(p)+'/save_net.ckpt')
        # print("Model saved in file: ", save_path)  # % save_path
        # print(sess.run(llr_froz_vec[0:5, :]))

        # keshan

        RELIA=sess.run(llr_froz_vec)
        chan_rel = normalization(RELIA, batch_size)
        rr = np.sum(chan_rel, axis=0) / batch_size
        # print(np.shape(rr))
        np.savetxt('D../Paramofpaperfig4/close/b0/t1.txt', rr)
...
def test0(p):
    with tf.Session() as sess:
        # tf.global_variables_initializer().run()
        saver.restore(sess, '../ParamOfSingleBNN/away/slot1/'+str(p)+'/save_net.ckpt')
        print('!!!!!!!!!!!!!!!!!!!!!!!! test0 !!!!!!!!!!!!!!!!!!!!!!!!!!\n')
        is_test = 1
        ...
        for i in range(t_step):
            ...
            # H_ch = np.load('../ParamOfSingleBNN/h/H/t1.npy')
            # s_H_ch = np.load('../ParamOfSingleBNN/h/s_H/t1.npy')
            #
            # H_ch4 = np.reshape(H_ch[:, 0, :], [batch_size, 1, -1])
            # s_H_ch4 = np.reshape(s_H_ch[:, 0, :], [batch_size, 1, -1])


            b0_mat = [0] * extranum
            ######################## #######
            y_pred, lab, ffro,ZZ,ZZZ,K,P_del,POSSS,RELIA = sess.run(fetches=[y_output, y_out_label, Fro,Z,Z2,kk,P_d,posss,llr_froz_vec],
                                               feed_dict={x: X_test, Kron: get_F_kron_n(n), H:H_ch4, s_H:s_H_ch4,
                                                          noisestd: np.sqrt(0.5), zero_mat: b0_mat,
                                                          lr: learning_rate, istest: is_test})
        ...
        pos_pro2 = pos_pro / (batch_size * t_step)
        test_Pdel=test_Pdel/t_step
    ...
    return test_ber / llen,test_Pdel
    #
    # ####################### save reliability #######################
    # chan_rel=normalization(RELIA,batch_size)
    # rr=np.sum(chan_rel, axis=0)/batch_size
    # # print(np.shape(rr))
    # np.savetxt('Paramofpaperfig4/away/b0/t1.txt', rr)
    #
    #
    # ##############################################
    # # print('x', zz)
