    ...
    with tf.Session() as sess:
        ...
        for j in range(epoch):
            ...
            for i in range(n_steps):
                ...
                H_ch4=np.reshape(H_ch3[:, 1, :], [batch_size, 1, -1])
                s_H_ch4=np.reshape(s_H_ch3[:, 1, :], [batch_size, 1, -1])
        ...
        # save_path = saver.save(sess, 'D:/cuijingwen/PycharmProjects/stage3/ParamOfSingleBNN/away/slot2/'+str(p)+'/save_net.ckpt')
        ...
        # print(np.shape(rr))
        np.savetxt('../Paramofpaperfig4/close/b1/t1.txt', rr)
...
def test1(p):
    with tf.Session() as sess:
        # tf.global_variables_initializer().run()
        saver.restore(sess, '../ParamOfSingleBNN/away/slot2/'+str(p)+'/save_net.ckpt')
        print('!!!!!!!!!!!!!!!!!!!!!!!! test1 !!!!!!!!!!!!!!!!!!!!!!!!!!\n')
        is_test = 1
        ...
        for i in range(t_step):
            ...
            #
            # H_ch4 = np.reshape(H_ch[:, 1, :], [batch_size, 1, -1])
            # s_H_ch4 = np.reshape(s_H_ch[:, 1, :], [batch_size, 1, -1])
