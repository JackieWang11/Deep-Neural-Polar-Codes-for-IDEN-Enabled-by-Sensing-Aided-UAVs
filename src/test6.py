...
llr_froz_vec=tf.Variable((np.float32(ffro)*0.1))#tf.Variable((np.float32(ffro))*0.1)#tf.Variable((np.float32(ffro)-0.5))#  远离：tf.Variable(initializer([batch_size, N])*10) tf.Variable(np.float32(pa)*0.1)  靠近：tf.Variable((np.float32(ffro)-0.5))
...
loss=1*tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=y_out_label,logits=y_output))+\
      1*tf.reduce_mean(tf.square(Rate-tf.reduce_mean(Prob,axis=1)))+0.05*tf.reduce_mean(Rate-tf.reduce_mean(Prob,axis=1))
...
with tf.Session() as sess:
    ...
    # # # # #
    # # # # #
    # save_path = saver.save(sess, 'D:/workspace/cuijingwen/stage3_2/ParamofFig3/close/256/3/save_net.ckpt')
    ...
    # print(np.shape(rr))
    # np.savetxt('reliabilitykao2/256/'+str(N)+'s'+str(SNR_db)+'e'+str(epoch)+'step50.txt', rr)
